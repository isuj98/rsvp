...
const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  // Nested tab for RSVP Results
  const [rsvpSubTab, setRsvpSubTab] = useState<'all' | 'attending'>('all');
  ...
  const handleDeleteSubmission = async (name: string) => {
    if (
      window.confirm(
        `Permanently remove ${name}'s RSVP record from MongoDB?`
      )
    ) {
      setLoading(true);
  ...
  const filteredSubsBase = Array.isArray(submissions)
  ...
  const filteredSubs =
    rsvpSubTab === 'attending'
      ? filteredSubsBase.filter((s) => s.isAttending === true)
      : filteredSubsBase;

  const filteredGuests = Array.isArray(guestList)
    ? guestList.filter((g) =>
  ...
  const declinedCount = submissions.filter((s) => s.isAttending === false)
    .length;
    ...
    <div className="min-h-screen bg-[#FDFBF7] p-6 md:p-12 selection:bg-[#F1CBA4] selection:text-[#5C3D2E]">
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-16 gap-8">
          <div>
            <h1
              className="text-6xl md:text-8xl font-script leading-tight"
            ...
            <div className="flex items-center gap-4 mt-4">
              <span
                className={`flex items-center gap-2 px-4 py-1.5 bg-stone-100 rounded-full text-[9px] uppercase tracking-widest font-black ${
          ...
          <button
            onClick={onLogout}
            className="px-10 py-4 bg-stone-900 text-white rounded-full text-[10px] uppercase tracking-[0.4em] font-bold hover:bg-black shadow-2xl transition-all transform hover:-translate-y-1"
          >
            Sign Out
          </button>
        </header>
        ...
        <nav className="flex space-x-12 mb-12 border-b border-stone-100 overflow-x-auto whitespace-nowrap scrollbar-hide">
          <button
            ...
            className={`pb-6 text-[11px] uppercase tracking-[0.5em] font-black transition-all relative ${
              activeTab === 'submissions' ? 'opacity-100' : 'opacity-20'
            }`}
          >
            RSVP Results
            {activeTab === 'submissions' && (
              <div className="absolute bottom-0 left-0 w-full h-1 bg-stone-800 rounded-t-full"></div>
            )}
          </button>
          <button
            onClick={() => {
            ...
            className={`pb-6 text-[11px] uppercase tracking-[0.5em] font-black transition-all relative ${
              activeTab === 'guestlist' ? 'opacity-100' : 'opacity-20'
            }`}
          >
            Guest Authorization
            {activeTab === 'guestlist' && (
        ...
        <div style={{ display: activeTab === 'submissions' ? 'block' : 'none' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <div className="bg-white border border-stone-100 shadow-sm rounded-sm p-8">
              ...
              <p
                className="text-5xl font-serif-elegant italic"
                style={{ color: COLORS.dark }}
              >
                {totalResponses}
              </p>
            ...
            <div className="bg-green-50 border border-green-100 shadow-sm rounded-sm p-8">
              <div className="flex items-center justify-between mb-4">
                <span
                  className="text-[10px] uppercase tracking-[0.5em] font-black opacity-60"
                  style={{ color: COLORS.dark }}
                >
                  Accepted
                </span>
              ...
              <p className="text-5xl font-serif-elegant italic text-green-700">
                {acceptedCount}
              </p>
            </div>
            <div className="bg-red-50 border border-red-100 shadow-sm rounded-sm p-8">
              ...
              <p className="text-5xl font-serif-elegant italic text-red-700">
                {declinedCount}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
            <div className="relative w-full max-w-md">
              ...
              />
              <svg
                className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 opacity-20"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <div className="overflow-x-auto custom-scroll">
              <table className="w-full text-left">
                <thead className="bg-stone-50 border-b border-stone-100">
                  <tr>
                    <th className="px-10 py-7 text-[10px] uppercase tracking-[0.4em] font-black opacity-30">
                      Status
                    </th>
                    <th className="px-10 py-7 text-[10px] uppercase tracking-[0.4em] font-black opacity-30">
                      Guest
                    ...
                    <th className="px-10 py-7 text-[10px] uppercase tracking-[0.4em] font-black opacity-30 text-right">
                      Delete
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-stone-50">
                  {filteredSubs.map((sub, i) => (
                    ...
                    >
                      <td className="px-10 py-10">
                        <span
                          className={`px-4 py-1.5 rounded-full text-[9px] uppercase tracking-widest font-black ${
                            sub.isAttending
                      ...
                      <td className="px-10 py-10">
                        <p className="font-serif-elegant italic text-3xl text-stone-900">
                          {sub.guestName}
                        </p>
                        {sub.message && (
                          <button
                      ...
                      <td className="px-10 py-10">
                        <div className="flex flex-wrap gap-2">
                          {sub.companions?.map((c, j) => (
                            <span
                              key={j}
                              className="text-[10px] bg-white border border-stone-200 px-3 py-1 rounded italic text-stone-500"
                            >
                              {c.name}
                            </span>
                          ))}
                      ...
                      <td className="px-10 py-10 font-mono text-[11px] opacity-40">
                        {sub.contactNumber}
                      </td>
                      <td className="px-10 py-10 text-right">
                        <button
                          onClick={() => handleDeleteSubmission(sub.guestName)}
                          className="text-stone-200 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all p-3"
                        >
                          <svg
                            className="w-6 h-6"
                            fill="none"
                  ...
                  {filteredSubs.length === 0 && (
                    <tr>
                      <td
                        colSpan={5}
                        className="text-center text-stone-400 py-14"
                      >
                        {rsvpSubTab === 'attending'
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="space-y-8">
              <div className="bg-white p-12 border border-stone-100 shadow-2xl rounded-sm">
                <h3 className="text-3xl font-serif-elegant italic mb-10">
                  Authorize New Guest
                </h3>
                <form onSubmit={handleAddGuest} className="space-y-10">
                  <div className="space-y-4">
                    <label className="text-[10px] uppercase tracking-[0.5em] font-black opacity-30">
                      Full Formal Name
                    </label>
                    <input
                      type="text"
                      required
                      className="w-full border-b border-stone-200 py-4 focus:outline-none focus:border-[#F1CBA4] font-serif-elegant italic text-2xl bg-transparent transition-all"
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-6 text-white uppercase tracking-[0.5em] text-[10px] font-bold shadow-2xl hover:bg-stone-800 transition-all transform hover:-translate-y-1 disabled:opacity-50"
              ...
              <div className="bg-white p-10 md:p-14 border border-stone-100 rounded-sm shadow-sm h-full flex flex-col">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
                  <div>
                    <h3 className="text-4xl font-serif-elegant italic mb-2">
                      Master Registry ({guestList.length})
                    </h3>
                    <p className="text-[10px] uppercase tracking-[0.2em] font-black opacity-30">
                      Authorized to RSVP via MongoDB
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 overflow-y-auto custom-scroll max-h-[60vh] pr-4">
                  {filteredGuests.map((name, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between p-5 bg-stone-50/50 rounded-sm group hover:bg-white hover:shadow-xl transition-all border border-transparent hover:border-stone-100/50"
                    >
                      <span className="text-base font-serif-elegant italic text-stone-800 tracking-wide font-medium">
                        {name}
                      </span>
                      <button
                        onClick={() => handleDeleteGuest(name)}
                        className="text-stone-200 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity p-2"
                      >
                        <svg
                          className="w-5 h-5"
                          fill="none"
      ...
      {selectedMessage && (
        ...
        >
          ...
          >
            ...
            <div className="mb-8">
              <span className="text-[9px] uppercase tracking-[0.4em] font-bold opacity-30 block mb-4">
                Message from
              </span>
              <h3
